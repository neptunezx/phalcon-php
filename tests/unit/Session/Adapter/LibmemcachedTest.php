...
class LibmemcachedTest extends UnitTest
{
    ...
    {
        $this->specify(
            "The session cannot be read or written from", function () {
    ...
    {
        $this->specify(
            "The session cannot be destroyed", function () {
            $sessionID = "abcdef123456";
    ...
    {
        $this->specify(
            'The libmemcached adapter does not clear session superglobal after destroy', function () {
            $session = new Libmemcached([
                'servers'  => [
                    [
                        'host' => env('TEST_MC_HOST', '127.0.0.1'),
