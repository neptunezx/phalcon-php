...
class FilesTest extends UnitTest
{
    ...
    {
        $this->specify(
            'The Files Adapter is handling the named session incorrectly', function () {
            $session = new Files();
    ...
    {
        $this->specify(
            'Headers not sent yet but the session can not be started', function () {
            $session = new Files();
    ...
    {
        $this->specify(
            'Session getter does not work correctly', function () {
            $session = new Files();
    ...
    {
        $this->specify(
            'Files session does not write data correctly', function () {
            $I    = $this->tester;
    ...
    {
        $this->specify(
            'Files session does not write data correctly with magic __set', function () {
            $I    = $this->tester;
            $path = PATH_OUTPUT . sprintf('tests%ssession%s', DIRECTORY_SEPARATOR, DIRECTORY_SEPARATOR);
    ...
    {
        $this->specify(
            'Files session does not read data correctly', function () {
            $I    = $this->tester;
    ...
    {
        $this->specify(
            'Files session does not read data correctly with magic __get', function () {
            $I    = $this->tester;
            $id   = md5(microtime(true));
            $path = PATH_OUTPUT . sprintf('tests%ssession%s', DIRECTORY_SEPARATOR, DIRECTORY_SEPARATOR);
            $file = sprintf('%ssess_%s', $path, $id);
    ...
    {
        $this->specify(
            'The files adapter does not clear session superglobal after destroy', function () {
            $session = new Files([
                'uniqueId' => 'session',
                'lifetime' => 3600,
            ]);
