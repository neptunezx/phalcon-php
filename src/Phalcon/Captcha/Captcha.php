...
/**
 * Phlacon\Captcha\Captcha
 * 
 * Generate captcha image
 * 
 * <code>
 * use Phlacon\Captcha\Captcha;
 * 
 * $captcha = new Captcha();
 * $captcha->CreateImage();
...
class Captcha implements CaptchaInterface
{
    ...
    {
        $optionKeys = [
            'width', 'height', 'minWordLength', 'maxWordLength',
            'backgroundColor', 'blur', 'scale', 'debug', 'imageFormat',
            'Yperiod', 'Yamplitude', 'Xperiod', 'Xamplitude', 'shadowColor',
            'lineWidth', 'Xperiod', 'Xamplitude', 'resourcesPath'
        ];
    ...
    public function CreateImage()
    {
        ...
        /** Text insertion */
        if (is_null($this->captchaText)) {
            $this->captchaText = $this->GenerateCaptchaText();
        }
        $fontcfg = $this->fonts[array_rand($this->fonts)];
        $this->WriteText($this->captchaText, $fontcfg);
        ...
        if ($this->debug) {
            imageString($this->im, 1, 1, $this->height - 8, "$this->captchaText {$fontcfg['font']} " . round((microtime(true) - $ini) * 1000) . "ms", $this->GdFgColor);
        }
     ...
     * @return string Text
     */
    protected function GenerateCaptchaText()
    {
     ...
     * @return string Text
     */
    protected function GetRandomCaptchaText($length = 4)
    {
        if (empty($length)) {
    ...
    function GetDictionaryCaptchaText($extended = false)
    {
        ...
        if (substr($this->wordsFile, 0, 1) == '/') {
            ...
            $wordsfile = substr($wordsfile, 0, 4);
        } else {
